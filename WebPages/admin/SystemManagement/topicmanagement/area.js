...
$('.setAreaBtn, .areaPlus').on('click', function() {
	...
	*/
	...
	var url = 'http://membership.ipaloma.com/webapi/ipaloma/topic/district/charge?district_type=province';
	_ajax("get", url, {}, '获取全省的负责人信息', function (dataprov){
		$('.layer-wait').remove();
		// c(dataprov);
...
function dataLoad() {
	...
	$('.area-list').on('click', '.selectable li', function() {
		...
		if (_this.closest('.Select_province').length == 1){
			...
			var url = 'http://membership.ipaloma.com/webapi/ipaloma/topic/district/charge?district_type=city&province=' + shengText;
			_ajax("get", url, {}, '获取省下所有市的负责人信息', function (datacity){
				$('.layer-wait').remove();
				// debugger
