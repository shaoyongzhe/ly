    ...
    _self.Bind = function() {
			...
			document.getElementById(_self.Setting.UpBtn).onchange = function() {
				if (this.value) {
                    ...
                    var f = document.getElementById(_self.Setting.UpBtn).files[0];
                    if (f.size > _self.Setting.fileSize * 1024)
                    {
                        alert("too large");
                        return false;
                    }else{
					...
					if (navigator.userAgent.indexOf("MSIE") > -1) {  //浏览器版本过滤 navigator.userAgent.indexOf()
						try {
